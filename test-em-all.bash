...
assertEqual $PROD_ID_REVS_RECS $(echo $RESPONSE | json_pp .productId)
assertEqual 3 $(echo $RESPONSE | json_pp ".recommendations | length")
assertEqual 3 $(echo $RESPONSE | json_pp ".reviews | length")
...
assertEqual "No product found for productId: $PROD_ID_NOT_FOUND" "$(echo $RESPONSE | json_pp -r .message)"
...
assertEqual $PROD_ID_NO_RECS $(echo $RESPONSE | json_pp .productId)
assertEqual 0 $(echo $RESPONSE | json_pp ".recommendations | length")
assertEqual 3 $(echo $RESPONSE | json_pp ".reviews | length")
...
assertEqual $PROD_ID_NO_REVS $(echo $RESPONSE | json_pp .productId)
assertEqual 3 $(echo $RESPONSE | json_pp ".recommendations | length")
assertEqual 0 $(echo $RESPONSE | json_pp ".reviews | length")

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl http://$HOST:$PORT/product-composite/-1 -s"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | json_pp .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl http://$HOST:$PORT/product-composite/invalidProductId -s"
assertEqual "\"Type mismatch.\"" "$(echo $RESPONSE | json_pp .message)"
